...
export const projectCache = (id) => join(projectDir(id), "cache");   // fetched abc-labs/ files, refreshed each sync
...
// Everything else Labs reads from a project comes from the same folder, under
// FIXED names — never a path the manifest points at. Labs therefore knows nothing
// about how a project is laid out inside, and a project that reorganises itself
// changes nothing here. The project puts what it wants shown into abc-labs/;
// how it gets there (copied by its build, written by its release script) is its
// own business.
export const LABS_FILES = {
  readme: "README.md",        // rendered on the project page — the project's own story
  changelog: "CHANGELOG.md",  // its newest section becomes "What's new"
  icons: ["icon.svg", "icon.png"],  // first one found wins; served as <img>, never inlined
};
...
export function validateExport(ex, expectId, platform) {
  ...
  if (ex.version !== undefined && (typeof ex.version !== "string" || !VERSION_RE.test(ex.version))) errors.push(`export: "version" must look like 1.2.3 (a v prefix and a -beta.1 suffix are fine) — have your release script write it, not a hand`);
  if (ex.requires !== undefined) {
    if (!Array.isArray(ex.requires) || ex.requires.length > 8 || ex.requires.some((r) => typeof r !== "string" || !r.trim() || r.length > 40)) {
      errors.push(`export: "requires" must be a list of up to 8 short phrases a user needs before this works, e.g. ["Tampermonkey", "Chrome or Edge"]`);
    }
  }
