...
export const DIRS = {
  projects: join(VAR, "projects"),   // guest checkouts
  registry: join(VAR, "registry.d"), // one <id>.json per listed project — the catalog and the index read only this
  routes: join(VAR, "routes"),       // <service>.caddy, generated; imported by infra/Caddyfile
  env: join(VAR, "env"),             // <id>.env, injected into the project's unit
};
...
export function readRealized() {
  if (!existsSync(DIRS.registry)) return [];
  return readdirSync(DIRS.registry)
    .filter((f) => f.endsWith(".json"))
    .sort()
    .map((f) => readJSON(join(DIRS.registry, f)));
}
export function writeRealized(entry) {
  ensureDirs();
  writeJSON(join(DIRS.registry, `${entry.id}.json`), entry);
}
export function removeRealized(id) {
  rmSync(join(DIRS.registry, `${id}.json`), { force: true });
}
...
export const CATEGORIES = {
  api:       { kind: "service",  name: "APIs",       test: "You send a request and get a result, without owning how.",        surfaces: ["mcp", "app", "cli", "package", "bot"] },
  tool:      { kind: "service",  name: "Tools",      test: "You operate it yourself to do your own work faster.",             surfaces: ["userscript", "extension", "app", "cli", "package", "mcp"] },
  agent:     { kind: "service",  name: "Agents",     test: "You hand it a goal; it works out the steps.",                     surfaces: ["mcp", "app", "cli", "package", "bot"] },
  component: { kind: "service",  name: "Components", test: "It becomes a part of your product.",                              surfaces: ["package", "import"] },
  knowledge: { kind: "resource", name: "Knowledge",  test: "It makes you, or your agent, know something: how, what, why.",    surfaces: ["ai", "import", "download"] },
  asset:     { kind: "resource", name: "Assets",     test: "Material you take and use as it is; it does nothing by itself.",  surfaces: ["download", "import", "package"] },
  template:  { kind: "resource", name: "Templates",  test: "You start your own thing from it.",                               surfaces: ["import", "app", "download"] },
};
...
export const SURFACES = {
  mcp:        { where: "an AI client, over MCP",                    fields: {},                                          needs: "uses.mcp" },
  ai:         { where: "an AI that reads its format",              fields: { format: "required" },                      needs: "install" },
  import:     { where: "abc-labs/<id>/ in a repository, vendored",  fields: {},                                          needs: "install" },
  userscript: { where: "the browser, via Tampermonkey",             fields: { url: "required" } },
  extension:  { where: "inside another application",                fields: { url: "required", host: "required" } },
  app:        { where: "an application you open",                   fields: { url: "required", platform: "optional" } },
  cli:        { where: "a terminal",                                fields: { command: "required" } },
  package:    { where: "a package registry",                        fields: { command: "required", registry: "optional" } },
  bot:        { where: "a chat",                                    fields: { url: "required" } },
  download:   { where: "a file you fetch",                          fields: { url: "required" } },
};
...
export function validateExport(ex, expectId, platform) {
  ...
  need("id", "string");
  if (typeof ex.id === "string" && !ID_RE.test(ex.id)) errors.push(`export: "id" must match ${ID_RE} — it becomes a URL path`);
  if (expectId && ex.id !== expectId) errors.push(`export: "id" is "${ex.id}" but registry.json lists this repo as "${expectId}"`);
  need("name", "string");
  need("tagline", "string");
  need("status", "string");
  ...
  if (ex.surfaces === undefined) errors.push(`export: missing "surfaces" — where does a user meet this? one or more of: ${Object.keys(SURFACES).join(", ")}`);
    ...
    for (const [name, def] of Object.entries(ex.surfaces)) {
      ...
      for (const [f, rule] of Object.entries(spec.fields)) {
        if (rule === "required" && (typeof def[f] !== "string" || !def[f].trim())) errors.push(`export: surfaces.${name} needs "${f}"`);
